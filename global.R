...
#library(reactable)
dashdata <- list()
dashdata$DB <- file.path(Sys.getenv("GETUIGENISSEN_2_0_SEARCH_HOME", unset = "/srv/shiny-server/getuigenissen/search"), "data", "DB.rds")
dashdata$DB <- readRDS(dashdata$DB)
dashdata$DB <- data.table::setDF(dashdata$DB)
...
dashdata$w2v       <- file.path(Sys.getenv("GETUIGENISSEN_2_0_SEARCH_HOME", unset = "/srv/shiny-server/getuigenissen/search"), "data", "w2v.bin")
